...
describe("Creating a preview image", () =>{
   test("Creating an image without a user ID should return 401", () =>{
        return request(app).post(`${previewRef}/${sceneID}`).attach("preview", image).expect(401);
   });

   test("Creating an image with a invalid file should return 400", () => {
        return request(app).post(`${previewRef}/${sceneID}`).attach("preview", invalid_image).set(validHeaders).expect(400);
   });
   ...
   test("Creating an image with the wrong user ID should return 401", () =>{
       return request(app).post(`${previewRef}/${sceneID}`).set({"x-access-token": "nope"}).attach("preview", image).expect(401);
   });
   test("Creating an image without an existing scene should return 404", () =>{
        return request(app).post(`${previewRef}/bobross`).set(validHeaders).attach("preview", image).expect(404);
   });
   test("Creating an image with everything valid should return 201", ()=> {
       return request(app).post(`${previewRef}/${sceneID}`).attach("preview", image).set(validHeaders).expect(201);
   });
   test("Creating an image with an existing preview image should return 409", () => {
        return request(app).post(`${previewRef}/${sceneID}`).attach("preview", image).set(validHeaders).expect(409);
   });
});

describe("Updating a preview image", () => {
    test("Updating an image without a user ID should return 401", () =>{
        return request(app).put(`${previewRef}/${sceneID}`).attach("preview", image).expect(401);
   });

   test("Updating an image with a invalid file should return 400", () => {
        return request(app).put(`${previewRef}/${sceneID}`).attach("preview", invalid_image).set(validHeaders).expect(400);
   });
   ...
   test("Updating an image with the wrong user ID should return 401", () =>{
       return request(app).put(`${previewRef}/${sceneID}`).set({"x-access-token": "nope"}).attach("preview", image).expect(401);
   });
   test("Updating an image without an existing scene should return 404", () =>{
        return request(app).put(`${previewRef}/bobross`).set(validHeaders).attach("preview", image).expect(404);
   });
   test("Updating an image with everything valid should return 204", ()=> {
       return request(app).put(`${previewRef}/${sceneID}`).attach("preview", image).set(validHeaders).expect(204);
   });
   test("Updating an image without an existing preview image should return 404", () => {
        return request(app).put(`${previewRef}/${altSceneID}`).attach("preview", image).set(validHeaders).expect(404);
   });
});
